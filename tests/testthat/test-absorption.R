...
test_that("Workflow generates appropriate files and folders", {
  expect_setequal(list.files(workflowAbs$workflowFolder), refWorkflowStructure)
})

test_that("Absorption directory includes appropriate files and folders", {
  expect_setequal(list.files(file.path(workflowAbs$workflowFolder, "Absorption")), absorptionStructure)
})
