...
A template R script for qualification workflows is available below or using the function `system.file("extdata", "qualification-workflow-template.R", package = "ospsuite.reportingengine")`.
...
The default reference word document can be found using `system.file("extdata", "reference.docx", package = "ospsuite.reportingengine")`.
...
In case, such reference is not defined, the qualification worklfow uses the **Id** of the section.
If no **Reference** nor **Id** is defined, the qualification worklfow generate a reference id named `"undefined-section-<section index>"`.
