...
In order to define and run appropriately a Worflow object, a few steps are required:
...
SimulationSet$new()
```
- 4. Define the worflow and its result folder
```r
Workflow$new()
```
- 5. Define the worflow tasks
```r
...
rmarkdown::render(myExampleWorkflow$reportFilePath)
```
...
knitr::raw_html(readLines(sub(".md", ".html", myExampleWorkflow$reportFilePath)))
```
</div>
