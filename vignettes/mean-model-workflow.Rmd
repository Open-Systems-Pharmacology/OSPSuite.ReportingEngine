...
The default value for this input is `NULL`, in which case `MeanModelWorkflow` will create a new directory using a default nomenclature:
...
Since some of the tasks are not always run or can use the results from tasks run in previous workflows, it is possible to activate/inactivate those before (re)running the workflow.
...
- `log-info`: save relevant messages from the workflow
- `error`: save warnings and errors messages
- `debug`: save most of messages which can be accessed for debugging purpose. 
...
## Workflow run
As defined above, only the simulation task is active and will be run.
```{r}
meanWorkflow$runWorkflow()
```

At the workflow level, the run has created the following structure within the `workflowFolder`:
```{r}
list.files(meanWorkflow$workflowFolder,
           include.dirs = TRUE)
```

At the task level, the run has also created its specific content:
```{r}
list.files(file.path(meanWorkflow$workflowFolder, meanWorkflow$simulate$outputFolder))
```

## Workflow on existing output

Re-initialize workflow structure with the same `meanModelSets` as previously defined"the list of simulation sets and re-use `"WorkflowTest01"` as workflow folder.
Since the workflow folder already exists, a warning message indicates that its content may be used or overwritten.
```{r}
meanWorkflow <- MeanModelWorkflow$new(simulationSets = meanModelSets, 
                                      workflowFolder = file.path(testFolder, "WorkflowTest01"))
...
Since `simulate` task was alread run, we will use the result to plot goodness of fit diagnostics by activating the `plotGoF` task.
```{r}
meanWorkflow$activateTasks(tasks = c("plotGoF"))
meanWorkflow$inactivateTasks(tasks = c("simulate"))
meanWorkflow$runWorkflow()
```

## Workflow report

The workflow run has created a report file using a .Rmd format.
If the task `renderReport` is activated, the workflow can compile the .Rmd file and render the report in any targeted format such as md, pdf, html or word.

The workflow is re-run below to include Mass Balance and Absorption assessments within the report.
```{r}
meanWorkflow$activateTasks(tasks = c("plotGoF", "plotMassBalance", "plotAbsorption"))
meanWorkflow$runWorkflow()
```
---

```{r child = meanWorkflow$reportFileName}
```}
