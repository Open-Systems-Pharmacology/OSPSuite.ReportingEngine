...
rmarkdown::render(mwf$reportFilePath)
```

<div class = "reportOutput">
```{r mwf report, echo=FALSE, results='asis'}
knitr::raw_html(readLines(sub(".md", ".html", mwf$reportFilePath)))
```
</div>
...
# Convert the markdown document into html
# Otherwise the root path is messed up and cannot print the report correctly
rmarkdown::render(pwf$reportFilePath)
```

<div class = "reportOutput">
```{r pwf report, echo=FALSE, results='asis'}
knitr::raw_html(readLines(sub(".md", ".html", pwf$reportFilePath)))
```
</div>
