...
The mean model workflow operates on `SimulationSet` objects, which combine the path to a `.pkml` file with `Output` objects (specified above) and which are given descriptive names (`simulationSetName`).  These objects are created as follows:   
...
A mean model workflow object is then instantiated thus:
...
In this workflow, the activities will consist of a sensitivity analysis and the generation of plots of the sensitivity analysis results.  To set this, the 
