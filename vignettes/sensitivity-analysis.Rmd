...
```{r} 
output1 <- Output$new(
  path = "Organism|Lung|Interstitial|Raltegravir|Concentration", displayName = "op1",
  pkParameters = c(
...
Here, `output1` and `output2` are `Output` objects.  The `path` input to the constructors of these objects gives the path to the model output.  For example, the `path` input for `output1` points to the concentration of Raltegravir in lung interstitial tissue, `Organism|Lung|Interstitial|Raltegravir|Concentration`.  This model output is given the display name `op1`. The PK parameters chosen for this example are the `C_max` and `MRT`, which are given the display names `new_C_max` and `new_MRT`, respectively.  Similarly, `output2` points to the model output with path `Organism|ArterialBlood|Plasma|Raltegravir|Concentration`.  The PK parameters selected for `output2` are `t_max` and `AUC_tEnd`, for which the respective display names `new_t_max` and `new_AUC_tEnd` will be used.
