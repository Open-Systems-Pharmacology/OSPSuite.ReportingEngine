...
A sensitivity analysis will compute the variation in a scalar function of a time-series trajectory of a model output in response to variations in model variables.  The model outputs are specified using paths to simulated quantities within the model.  The scalar functions are standard PK parameter quantities such as `C_max` or `AUC`.  Although the sensitivity analysis for a mean model or an individual within a population is evaluated for all PK parameters, the user has the option of specifying the subset of PK parameters for which sensitivity plots are to be generated.  In addition, the user has the option to set custom display names for both the selected model outputs and PK parameters used in the sensitivity analysis.  

The simplest way of defining a set of output and PK parameter combinations for the sensitivity analysis are as follows:

```{r} 
output1 <- Output$new(path = "Organism|VenousBlood|Plasma|Raltegravir|Concentration", displayName = "op1")
```

Here, `output1` is an `Output` object.  The `path` input to the constructor of this object gives the path to the model output that simulates the concentration of Raltegravir in venous blood plasma, `Organism|VenousBlood|Plasma|Raltegravir|Concentration`.  This model output is given the display name `op1`.  If sensitivity plots for specific PK parameters of this output are to be generated, these would also be specified at this stage.  However, in this case, no PK parameters are specified and therefore the reporting engine will, by default, generate sensitivity plots for __all__ PK parameters that can be evaluated for the model output `op1`. VERIFY IF THIS IS TRUE FOR MEAN MODEL - CURRENTLY ONLY TRUE FOR POPULATION.  IF NO PK PARAMETERS SELECTED FOR MEAN MODEL, NO PLOTS ARE GENERATED.
...
In this mean model workflow, the tasks will consist of a sensitivity analysis followed by generation of plots of the sensitivity analysis results.  These two task are activated as follows: 
...
In this example, the sensitivity analysis will be conducted for two populations in parallel.  In the first step, the model output to be analyzed are chosen:
...
With this setup, the sensitivity of PK parameters of `output1`, `output2` and `output3` will be analyzed for the first population (given in `populationFile1`).  For the second population (given in `populationFile2`), sensitivity will only be analyzed for PK parameters of `output3`.  The two `PopulationSimulationSet` are then used to create a `PopulationWorkflow`, the results of which are to be placed in the root directory `./popSensitivityExample`:
