...
nomenclatureTable <- data.frame(
  "R6 object" = c(
    "MeanModelWorkflow", "PopulationWorkflow", "SimulationSet", "PopulationSimulationSet",
    "Output", "Task", "SimulationTask", "PlotTask", "PopulationPlotTask", "GofPlotTask"
  ),
  "Mandatory Inputs" = c(
    "simulationSets<br>workflowFolder",
    "workflowType<br>simulationSets<br>workflowFolder",
    "simulationSetName<br>simulationFile",
    "simulationSetName<br>simulationFile<br>populationFile",
    "path",
    rep("settings", 5)
  ),
  "Role" = c(
    "Runs tasks on simulation sets dedicated to mean models",
    "Runs tasks on simulation sets dedicated to population models",
    "Defines the simulation and observation to be evaluated",
    "Defines the simulation, population and observation to be evaluated",
    "Defines the simulation output path and observation to be evaluated",
    "Defines evaluations to be performed and saved",
    "Defines evaluations to be performed and saved<br>specific to model simulations",
    "Defines evaluations to be performed and saved<br>specific to plots and tables to report",
    "Defines evaluations to be performed and saved<br>across simulations and specific to plots and tables to report",
    "Defines evaluations to be performed and saved<br>specific to plots and tables of goodness of fit to report"
  ),
  check.names = FALSE
)
...
## 2.4. Workflow built in methods and properties
...
Writing a workflow R script can be challenging, especially for R beginners.
The Excel template provides a simple interface to define the workflow features which can be translated into a documented R script using the function `createWorkflowFromExcelInput(excelFile)`.

The Excel template can be found using the function `system.file("extdata", "WorkflowInput.xlsx", package = "ospsuite.reportingengine")` as illustrated below.

```{r Excel template} 
excelFile <- system.file("extdata", "WorkflowInput.xlsx", package = "ospsuite.reportingengine")
```

Within the template, the following sheets are available and need to be updated:

- **Documentation** defines commented lines to appear at the beginning of the R sccript.
- **Workflow and Tasks** defines the workflow mode and type, as well as the active tasks and their settings.
- **SimulationSets** defines the simulation sets, their attached dataset and the sheet in which their outputs are defined (e.g. **Outputs_1**).
- **Outputs_1** (optional) defines an output and the sheet in which its PK parameters are defined (e.g. **PK Parameter**).
- **PK Parameter** defines the PK parameters of the output. A template for user-defined PK parameters is available in sheet **Userdef PK Parameter**.

The default file created by the function is `workflow.R`, but a different filename can be specified with the argument.
The example below also generates warnings in case of ill constructed workflows.

```{r Excel template call} 
createWorkflowFromExcelInput(excelFile, workflowFile = "excel-workflow.R")
```

The corresponding R script is printed below.
Note that the R script is commented to guide users if they wish to update the workflow settings.
The argument `removeComments` can be used to remove all the comments from the generated R script.

```{r Excel template script output, echo=FALSE, comment=NA} 
cat(readLines("excel-workflow.R", encoding = "UTF-8"), sep = "\n")
unlink("excel-workflow.R", recursive = TRUE)
```
