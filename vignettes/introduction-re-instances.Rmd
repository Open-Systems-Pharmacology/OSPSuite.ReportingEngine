...
myExampleWorkflow$activateTasks(c("simulate", "plotGoF"))
...
- __SensitivityResults__ which contains the results of sensitivity analysis runs, obtained from `sensitivityAnalysis` task, as csv files.
It is possible to load the results using the `ospsuite` method __importSensitivityAnalysisResultsFromCSV()__.
- __TimeProfiles__ which contains the figures obtained from `plotGoF` task and their source values as png and csv files.
...
- __appendix-time-profile.md__ from `plotGoF` task.
