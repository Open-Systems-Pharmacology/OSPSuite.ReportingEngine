...
If the simulation set corresponds to multiple administrations, evaluations on some application ranges can be switched on/off.
The input `applicationRanges` is a logical value or a list of logical values that can switch on/off the presence of some application ranges in the reported plots.
The field includes the following elements: `total`, `firstApplication` and `lastApplication`.
