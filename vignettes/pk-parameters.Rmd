...
Results obtained from the `calculatePKParameters` task and stored as csv files within the subdirectory __PKAnalysisResults__ from the `workflowFolder` directory are required in order to perform the goodness of fit (`plotPKParameters`) task.
...
the tasks `simulate` and `meanModelPKParameters` need to be performed.
The task `simulate` needs to be performed because the task `meanModelPKParameters` is using the time profile simulation results.
...
workflow4$inactivateTasks(c("simulate", "meanModelPKParameters"))
...
# Set the workflow tasks to be run
workflow5$activateTasks(c("simulatePopulation", 
...
# Set the workflow tasks to be run
workflow6$activateTasks(c("simulatePopulation", 
                          "populationPKParameters", 
                          "plotPKParameters"))
