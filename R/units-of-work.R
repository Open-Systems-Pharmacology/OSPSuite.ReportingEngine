...
plotDemography <- function(simulation,
                           population,
                           parameterNames = c(ospsuite::StandardPath$Age, ospsuite::StandardPath$Weight, ospsuite::StandardPath$Height),
  ...
  demographyValues <- as.data.frame(lapply(demographyParameters,
                                           function(p) {ospsuite::toDisplayUnit(p, population$getValues(p))}
                                           )
                                    )
...
plotGoodnessOfFit = function(populationSimulation,
  ...
  timeProfileResults <- ospsuite::getOutputValues(populationSimulation,
                                                  path,
  ...
  residualsData <- getResiduals(timeProfileResults$data,
                                observedData,
                                dataMapping = tlf::XYDataMapping$new(x = "Time",
                                                                y = quantity))
...
getResiduals <- function(simulatedData,
  ...
  dataMapping <- dataMapping %||% tlf::XYDataMapping$new(x = "Time",
                                                    y = ncol(observedData))
