...
getOutputsTimeProfile = function(plot){
...
#' @title getOutputsGOFMergedPlotsPlot
...
getOutputsGOFMergedPlotsPlot <- function(plot){
...
#' @title getScheduleOfPlotsOfType
#' @description Get a dataframe of plots of the plot type `plotType` to be generated from the `ConfigurationPlan` object that is generated by the configuration plan json
#' @param pltList is a list of plots that is to be generated, all of the plot type `plotType`
#' @param plotType describes the type of the plots described in `pltList`
#' @return A dataframe describing the the the order of each plot among its plot type `plotType`, and the section id of each plot
getScheduleOfPlotsOfType <- function(pltList,plotType){
  validateIsString(object = plotType)
  schedule <- NULL
  plotNumber <- 0
  for (plt in pltList){
    plotNumber <- plotNumber + 1
    schedule <- rbind.data.frame(schedule,data.frame(plotType = plotType,plotNumber = plotNumber,sectionId = plt$SectionId))
  }
  return(schedule)
}
...
#' @title getScheduleOfPlots
#' @description Get a dataframe of plots from the `ConfigurationPlan` object `configurationPlan` that is generated by the configuration plan json
#' @param configurationPlan is a `ConfigurationPlan` object that is generated by the configuration plan json
#' @return A dataframe describing the the type of each plot, the order of each plot among its plot type, and the section id of each plot
getScheduleOfPlots <- function(configurationPlan){
  validateIsIncluded(values = "plots",parentValues = names(configurationPlan),nullAllowed = FALSE)
  plotTypes <- setdiff(names(configurationPlan$plots),c("PlotSettings","AxesSettings"))
  sapply(plotTypes,function(x){validateIsIncluded(values = x,parentValues = qualificationPlotTypes)})
  schedule <- NULL
  for (plotType in plotTypes){
    pltList <- configurationPlan$plots[[plotType]]
    schedule <- rbind.data.frame(schedule,getScheduleOfPlotsOfType(pltList = pltList, plotType = plotType))
  }
  return(schedule)
}
