...
getQualificationPKRatioTable <- function(data, metaData, settings){
  ...
  metaData <- metaData[sapply(metaData, function(metaDataVariable){is.list(metaDataVariable)})]
  columnNames <- sapply(metaData, function(metaDataVariable){tlf::getLabelWithUnit(metaDataVariable$dimension, metaDataVariable$unit)})
...
getQualificationPKRatioData <- function(pkRatioPlan, configurationPlan, logFolder){
  ...
  pkRatioMetaData <- c(
    ...
    list(
      caption = caption,
      color = sapply(pkRatioPlan$Groups, FUN=function(group){group$Color %||% "black"}),
      shape = sapply(pkRatioPlan$Groups, FUN=function(group){tlfShape(group$Symbol) %||% tlf::Shapes$circle})
    )
  )
  return(list(
    data = pkRatioData,
